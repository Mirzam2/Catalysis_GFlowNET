...
```bash
cd /beegfs/home/georgii.bychkov/projects
git clone https://github.com/Mirzam2/Catalysis_GFlowNET.git pdh-gfn && cd pdh-gfn
pip install -e . --no-deps   # ОБЯЗАТЕЛЬНО --no-deps: pyproject объявляет полный
                             # стек (torch/fairchem/gflownet), без флага pip полезет
                             # ставить torch и сломает валидированное UMA-окружение
```
